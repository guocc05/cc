...
cc 原本的交互模式是"用户主动发消息 → AI 响应"。本功能要让 IM 端能挂一个"定时器"，到点自动以用户身份给本地某个 session 投递一条消息，触发 AI 在指定时刻继续工作——典型场景是配额窗口续接、晨会准备、周期性维护任务。
...
[设置]
   ...
   ↓
[持久化]
   ~/.cc/data/schedules.json（绝对时间戳 nextFireAt）
   ↓
[触发]
