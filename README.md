...
cc 让你在手机上通过飞书或微信，继续操控电脑上正在运行的 `Claude Code`、`Codex` 或 `Gemini CLI`。不是远程桌面，不是复制一份聊天记录，而是接着同一个本地 session 往下做。
...
`cc onboard` 会引导你接通飞书或微信、启动守护进程、创建首次会话、完成开机自启和安全加固。首次运行时 `onboard` 会自动把 `fn / fc / fl` 这类终端命令写入 `~/.zshrc`，并把 Claude Code session 同步 hook 写入 `~/.claude/settings.json`，这两步也可以单独执行（见下）。
...
```text
请帮我安装并配置 cc：先跑 npm i -g cc，再跑 cc onboard，然后按照引导接通飞书或微信即可。如果 npm 全局目录权限不足，指导我配置 ~/.npmrc 的 prefix（比如 ~/.npm-global）。
```
...
内部等价于 `npm i -g cc@latest`，再自动重启守护进程。你的运行时配置保存在 `~/.cc/`，更新不会覆盖这部分内容。

（如果你是从源码安装的贡献者，`cc update` 会给出 git 更新的引导；或者改用 npm 分发版。）
...
`cc setup` 现在只负责保存飞书凭证。把 Bot 加入飞书群后，直接按 `cc onboard` 的提示继续完成启动、真实会话接入，以及成功后的稳定化配置。
...
微信接通后，同样直接按 `cc onboard` 的提示继续完成启动、真实会话接入，以及成功后的稳定化配置。

## Onboarding 流程

`cc onboard` 会把首用流程拆成三段：先完成第一次成功，再做稳定化，最后再去学进阶能力。
...
接入后，cc 会先发送一条接入状态，再把最近一轮对话回顾发到手机端；默认优先保留最近一轮完整问答，并使用 `【你】` / `【AI】` 标签区分双方内容。若最近一轮过长，会在总数不超过 3 条消息的前提下自动拆分。
...
- **IM 用户白名单**：哪些 IM 用户可以给 bot 发控制命令（这是 cc 真正的认证边界）
...
`doctor` 会检查当前状态并给出下一步建议；高频命令速查用 `cc help`。
...
| `Codex` | 正式支持 | 通过 `cc` 创建并注册的会话可完整流转 |
| `Gemini CLI` | Best-effort | 通过 `cc` 创建并注册的会话可完整流转 |
...
命令按**作用对象**分两类。第一类是 cc daemon 自己处理的命令（会话/调度/状态），跟你用哪个 AI 工具无关；第二类是会话控制命令，daemon 拦截后操作当前对话所用的 AI 工具，**不同工具支持情况有差异**。

### cc 自有命令（daemon 直接处理，所有工具一致）
...
| `cc onboard` | 查看首次安装引导 | `cc onboard` | — |
| `cc secure` | 配置 IM 用户白名单 | `cc secure` | — |
| `/mode` | 查看可用模式 | — | `/mode` |
...
| `fqs` | 查看反茄钟状态 | `fqs` | `/fqs` |
| `cc help` / `fhelp` | 查看帮助 | `cc help` 或 `fhelp` | `/fhelp` |
...
- 查看帮助时，电脑终端优先用 `cc help`；`fhelp` 只是快捷命令。飞书/微信里用 `/fhelp`。旧的 `/help` 仍兼容，但不再作为主要入口。
- AI 工具自身的其他 slash 命令（如 `/init` `/memory` `/hooks` `/mcp` `/agents`）**不被 cc 拦截**，会作为普通 prompt 传给 AI，绝大多数会被 AI 工具拒绝执行——属于已知限制。
...
默认情况下，`cc` 会直接调用系统里的 `claude` 命令，这也是开源用户的默认路径。

如果你在自己电脑上有一个本地 Claude 启动脚本，想让 `fn` 创建 Claude 对话前先弹出“选择渠道 / profile”的菜单，可以在 `~/.cc/config.json` 里额外加一项：
...
cc 完全在你自己的电脑上运行，你的代码和对话内容不会经过任何第三方服务器。飞书和微信仅用于传递消息文本。

### 安全模型

cc 把 IM 消息等效于本地终端命令，因此有两层保护：

1. **谁能用（认证层）**：`allowedUserIds`（IM 用户白名单）限制只有你自己能发指令给 Bot——这是 cc 真正的认证边界，`cc secure` 配置。
2. **AI 能做什么（授权层）**：由各 AI 工具自身的 permission mode 决定。YOLO/bypassPermissions 模式下 AI 对文件系统无限制；auto/acceptEdits 模式下工具层会弹确认。

**重要**：cc 不做"路径白名单"式的沙箱——AI 启动后可用绝对路径访问机器上任意文件，路径限制给不了真正的安全保护。如果你用的是 YOLO/bypass 模式，请务必设置 `allowedUserIds` 并把 Bot 放在只有你自己能访问的 IM 会话里。
...
**在手机上操作时，电脑上的工具会怎样？**
> cc 保证同一时刻只有一个端在操控当前对话。当你在手机上接入对话时，电脑端会自动断开。回到电脑端接回时，飞书会收到通知；微信受平台限制，不保证总能主动推送提醒。不会出现两边同时操作导致冲突的情况。
>
> 特殊情况：如果你切回电脑端 `fc` 时，IM 端还有一个任务正在执行，电脑端不会立即进 tmux 而是进入"接回保护态"——显示一个监控面板等任务结束（或按 Ctrl-C 主动取消该任务再进入）。这是为了避免你刚接回就被 IM 端任务的输出打断。

**守护进程崩溃了怎么办？**
> 运行 `cc start` 即可（会自动清理残留状态）。cc 会自动恢复之前正在执行的任务，如果任务因重启中断，会在飞书/微信中通知你。

**微信 token 过期了怎么办？**
> 运行 `cc wechat login` 重新扫码绑定，然后 `cc stop && cc start` 重启守护进程。

**如何查看环境是否配置正确？**
> 运行 `cc doctor`，它会检查当前状态并给出下一步建议；如果你想看完整首用引导，运行 `cc onboard`。
...
cc logs               # 查看日志
cc onboard            # 首次安装引导
cc secure             # 配置 IM 用户白名单
cc doctor             # 环境检查
cc install-shell      # 写入 fn/fc/fl 终端命令
cc install-hook       # 写入 Claude Code session 同步 hook
cc install-service    # 安装 macOS 开机自启
cc update             # 更新到最新 (npm i -g cc@latest + 重启 daemon)
cc wechat login       # 绑定微信
cc wechat status      # 查看微信状态
cc wechat logout      # 解绑微信
```
...
cc 在你的电脑上运行一个轻量守护进程，它同时连接飞书和微信，把你的消息转发给本地 CLI，再把回复发回手机。当前正式支持 `Claude Code`、`Codex`，并提供 `Gemini CLI` 的 best-effort 支持。它直接操控 CLI，而不是走 Agent SDK 中转，所以工具原生的读写文件、执行命令、调用 MCP 等能力可以直接复用。

补充说明：当前“扫描并导入未注册的本地历史会话”主要面向 `Claude Code`；`Codex/Gemini` 的完整支持路径是通过 `cc new --tool ...` 或 IM 里的 `/fn --tool ...` 创建并注册后再流转。

## 从源码开发（贡献者）

如果你想修改 cc 源码、提 PR，走 npm link 工作流：

```bash
git clone https://github.com/JVever/cc.git
cd cc
bash install.sh              # npm install + npm run build + npm link
cc install-shell          # 写入终端命令
cc install-hook           # 写入 Claude hook
cc onboard
```

之后改 `src/*.ts` → `npm run build` → 重启 daemon 即可。想回到 npm 分发版：`npm unlink -g cc && npm i -g cc`。
