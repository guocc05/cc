...
const testHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cc-project-index-'))
...
test('listProjectIndex derives unique project list from registry cwds', () => {
  ...
  registry.register('auth', 'auth-session', '/Users/dev/Code/cc', 'claude')
  registry.register('bug', 'bug-session', '/Users/dev/Code/cc', 'claude')
  ...
  assert.deepEqual(labels, ['cc', 'portal'])
})

test('listProjectIndex disambiguates same-basename cwds with parent path', () => {
  resetState()

  // 两个不同的父目录下都有 "01-cc" → label 应降级为带父目录
  registry.register('a', 'a-session', '/Users/dev/Code/16-remote/01-cc', 'claude')
  registry.register('b', 'b-session', '/Users/dev/Code/17-other/01-cc', 'claude')

  const entries = projectIndex.listProjectIndex()
  assert.equal(entries.length, 2)
  const labels = entries.map(e => e.label).sort()
  assert.deepEqual(labels, ['16-remote/01-cc', '17-other/01-cc'])
})
...
test('resolveProjectHint matches unique short name (exact basename)', () => {
  resetState()
  registry.register('a', 'a-session', '/Users/dev/Code/cc', 'claude')
  registry.register('b', 'b-session', '/Users/dev/Code/portal', 'claude')

  const ok = projectIndex.resolveProjectHint('cc')
  assert.equal(ok.kind, 'ok')
  assert.equal(ok.cwd, '/Users/dev/Code/cc')
})
...
test('resolveProjectHint matches unique prefix', () => {
  resetState()
  registry.register('a', 'a-session', '/Users/dev/Code/cc-core', 'claude')
  registry.register('b', 'b-session', '/Users/dev/Code/portal', 'claude')

  const ok = projectIndex.resolveProjectHint('im2')
  assert.equal(ok.kind, 'ok')
  assert.equal(ok.cwd, '/Users/dev/Code/cc-core')
})

test('resolveProjectHint returns ambiguous when basename matches multiple cwds', () => {
  resetState()
  registry.register('a', 'a-session', '/Users/dev/Code/16-remote/cc', 'claude')
  registry.register('b', 'b-session', '/Users/dev/Code/17-other/cc', 'claude')

  const outcome = projectIndex.resolveProjectHint('cc')
...
test('suggestProjectLabels returns fuzzy matches from registry', () => {
  ...
  const suggestions = projectIndex.suggestProjectLabels('ccx')
  assert.ok(suggestions.includes('cc'))
})
